...
import { getApartmentById, updateApartmentStatus, updatePaymentStatus } from "@/api/apartament.js";
	...
	setTimeout(async () => {
		try	{
			...
			if (res.data.length <= 0) {
				...
				card.value = res.data;
				status.value = res.data.payment.statusPayment;
			}
		} catch (error) {
...
updatePaymentStatus(props.id, { status });

const apartmentStatus = computed(() => {
	if (card.value && card.value.status) {
		return card.value.status.statusOccupancy ? "СВОБОДНО" : "ЗАНЯТО";
	}
	return "";
});

const updateStatus = () => {
	updatePaymentStatus(props.id, { statusPayment: status.value })
		.then(() => {
